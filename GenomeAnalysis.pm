...
sub gff_parser{
	...
	my $gff_fh;
	if($gff_file=~/\.gz$/){
		open($gff_fh, "zcat $gff_file |") or die "Cannot open $gff_file: $!";
	}else{
		open($gff_fh, $gff_file) or die "Cannot open $gff_file: $!";
	}
	...
	foreach (@contents){
		...
		if($gff_file=~/gff/){
			...
			$transcript_id =(split/,/, $parent_attr)[0];
		}elsif($gff_file=~/gtf/){
			my $parent_attr =(split/;/, $attr)[0];
			$parent_attr =~s/\"//g;
			if($parent_attr=~/\s/){
				$transcript_id = (split/\s+/, $parent_attr)[1];	
			}else{
				$transcript_id = $parent_attr;
			}
		}
